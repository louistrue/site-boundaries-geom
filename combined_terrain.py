...
def _create_extruded_building_shell(model, polygon, base_z, height, offset_x, offset_y, offset_z):
    """Create a closed shell for an extruded building volume, returning grouped faces."""
    if height is None or height <= 0:
        return None, 0, [], [], []
    ...
    for i in range(len(local_ext) - 1):
        ...
        face = model.createIfcFace([model.createIfcFaceOuterBound(side_loop, True)])
    ...
    bottom_face = model.createIfcFace([model.createIfcFaceOuterBound(bot_loop, True)])
    footprint_faces.append(bottom_face)
    ...
    return shell, len(all_faces), roof_faces, facade_faces, footprint_faces
...
def create_combined_ifc(terrain_triangles, site_solid_data, output_path, bounds, 
                        center_x, center_y, egrid=None, cadastre_metadata=None, buildings=None, building_mode="solid"):
    """
    ...
    def _surface_rep_from_faces(face_list):
        if not face_list:
            return None
        shell = model.createIfcOpenShell(face_list)
    ...
    if buildings:
        ...
        for idx, building in enumerate(buildings, 1):
            ...
            if building.get("has_z"):
                ...
                if faces:
                    if building_mode == "separated":
                        roof_rep = _surface_rep_from_faces(faces)
                    else:
                ...
                for poly in polygons:
                    shell, face_count, roof, facades, footprint = _create_extruded_building_shell(
                        model, poly, building.get("base_z", 0.0), building.get("height", 0.0),
                        offset_x, offset_y, offset_z
                    )
                    total_faces += face_count
                    roof_faces.extend(roof)
                    facade_faces.extend(facades)
                    footprint_faces.extend(footprint)
                    if shell:
                        rep_items.append(model.createIfcFacetedBrep(shell))

                if rep_items and building_mode == "solid":
                    shape_rep = model.createIfcShapeRepresentation(
                        body_context, "Body", "Brep", rep_items
                    )
                    rep = model.createIfcProductDefinitionShape(None, None, [shape_rep])
                    print(f"  Building {idx}: created {total_faces} faces from {len(rep_items)} footprint(s)")
                elif building_mode == "separated":
                    roof_rep = _surface_rep_from_faces(roof_faces)
                    facade_rep = _surface_rep_from_faces(facade_faces)
                    footprint_rep = _surface_rep_from_faces(footprint_faces)

            if building_mode == "separated":
                # Create up to three child geographic elements
                def _attach_sub_element(name_suffix, shape_rep):
                    if not shape_rep:
                        return None
                    sub_elem = ifcopenshell.api.run(
                        "root.create_entity", model, ifc_class="IfcGeographicElement",
                        name=f"{building_name}_{name_suffix}"
                    )
                    sub_elem.OwnerHistory = owner_history
                    sub_elem.PredefinedType = "USERDEFINED"
                    sub_elem.ObjectType = f"Building_{name_suffix}"
                    sub_elem.ObjectPlacement = building_elem.ObjectPlacement
                    sub_elem.Representation = model.createIfcProductDefinitionShape(None, None, [shape_rep])
                    ifcopenshell.api.run("spatial.assign_container", model,
                                          products=[sub_elem], relating_structure=site)
                    return sub_elem

                roof_obj = _attach_sub_element("Roof", roof_rep)
                facade_obj = _attach_sub_element("Facades", facade_rep)
                footprint_obj = _attach_sub_element("Footprint", footprint_rep)
                if not any([roof_obj, facade_obj, footprint_obj]):
                    print(f"  Building {idx}: No separated geometry created")
            else:
                if rep:
...
def main():
    ...
    parser.add_argument("--building-elements", choices=["solid", "separated"], default="solid",
                        help="Export buildings as solids (default) or separated roof/facade/footprint surfaces")
    ...
    create_combined_ifc(
        ...
        buildings=buildings_prepared, building_mode=args.building_elements
    )
